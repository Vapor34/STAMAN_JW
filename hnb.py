...
class GraspPlanner(Annealer):
    ...
    def energy(self):
        ...
        for b_id in self.finger_tip_body_ids:
            avg_dist += np.linalg.norm(self.data.xpos[b_id] - bottle_pos)
        avg_dist /= len(self.finger_tip_body_ids)

        # 2. 朝向能量 (Palm Z-axis or Y-axis depending on model, assuming -Y here based on previous code)
        # 获取手掌旋转矩阵
        rot_mat = self.data.xmat[self.palm_body_id].reshape(3, 3)
        palm_normal = -rot_mat[:, 1] # 假设 -Y 是手心朝向
        ...
        for i in range(self.data.ncon):
            ...
            if is_bottle and is_hand:
                # 允许极轻微的接触(用于贴合)，但惩罚深度穿模
                if con.dist < -0.002: 
                    collision_penalty += 100.0

        return avg_dist * 10.0 + orient_energy + pose_energy + collision_penalty
